...
const PROMPT = {
  ...
  yetkiBlogu(proje) {
    ...
    s.push('**Veritabanı tarafı — SQL\'i sen çalıştıramıyorsun.** `kullanicilar`');
    s.push('tablosu bu depoda zaten var ama **personel listesi** olarak —');
    s.push('muhtemelen dolu (isimler girilmiş) ve `auth.users`\'a hiç bağlı');
    s.push('değil, id\'leri rastgele üretilmiş. Aşağıdakileri SQL olarak,');
    s.push('kopyalanabilir **tek bir blok** halinde ver; ben Supabase SQL');
    s.push('Editör\'e yapıştırıp çalıştıracağım, kodu commit\'e gönderdiğinde');
    s.push('SQL\'i unutma. **Sıra önemli — önce bu SQL çalışmalı, sonra RLS');
    s.push('kısmı ("RLS şimdilik..." maddesi) devreye girmeli; tersi olursa');
    s.push('kimse giremeyen bir uygulama kalır.**');
    s.push('- `kullanicilar` tablosuna **yeni bir `auth_id` kolonu** ekle —');
    s.push('  var olan `id` kolonuna dokunma, değiştirme. Mevcut satırların');
    s.push('  imza geçmişi (`degisiklik_kaydi` vb.) hâlâ eski `id`\'ye bağlı;');
    s.push('  onu bozarsan geçmiş kayıtlar sahipsiz kalır.');
    s.push('- Katman kolonu (ve varsa şube kolonu) ekle.');
    s.push('- Bir **trigger**: `auth.users`\'a yeni bir kayıt düştüğünde çalışsın.');
    s.push('  `kullanicilar`\'da katmanı en üst olan (Admin) bir satır **yoksa**,');
    s.push('  yeni bir satır aç (`auth_id` bu yeni kayda, katman en üste) —');
    s.push('  bu, panelden elle açtığım ilk admin hesabı için. Zaten bir admin');
    s.push('  varsa (yani bu ilk kez değilse), yine yeni bir satır aç ama en');
    s.push('  düşük katman + **pasif** olarak — böylece panelden yanlışlıkla ya');
    s.push('  da elle açılan bir hesap sahipsiz kalıp boş ekranda kilitlenmez.');
    s.push('  Trigger\'ı `on conflict (auth_id) do nothing` ile yaz: bu satırı');
    s.push('  gerçek bilgiyle dolduran aşağıdaki Edge Function\'ın kendi');
    s.push('  `upsert`\'üyle (`on conflict (auth_id) do update`) çakışmasın.');
    s.push('- `alter default privileges` ile yeni açılan tablolara otomatik');
    s.push('  `anon` yetkisi veren bir kural bu depoda zaten kurulu — onu');
    s.push('  **geri al**, yoksa bundan sonra eklenen her tablo sessizce');
    s.push('  anon\'a açık doğar.');
    s.push('- SQL bloğunun sonuna kısa bir **doğrulama sorgusu** ekle (ör.');
    s.push('  "bu sorgu 0 satır dönmeli" gibi bir not ile) — ben çalıştırıp');
    s.push('  gerçekten doğru kurulduğunu göreyim.');
    s.push('');
    ...
    s.push('Yukarıdaki trigger o hesabı otomatik admin yapacak, var olan bir');
    s.push('personel satırıyla eşleştirmeye çalışma — yeni bir satırdır. Bu');
    s.push('yüzden **kodun içine sabit bir kullanıcı adı/şifre gömme,');
    s.push('"veritabanı boşken şunu kabul et" gibi özel bir giriş yolu da');
    s.push('yazma** — login ekranı sıradan e-posta+şifre ile Supabase Auth\'a');
    s.push('bağlansın. Hiç admin yokken zaten kimse giremez; bu normal ve');
    s.push('beklenen bir durum, hata değil. `nizam/NIZAM.md`\'ye kısa bir not');
    s.push('düş: ilk admin hesabı Supabase panelinden açılıyor, uygulamada');
    s.push('kayıt ekranı yok.');
    s.push('');

    s.push('**"Kim yaptı" imzası artık gerçek girişten okunacak.** Bu depoda');
    s.push('şu ana kadar bir "personel seç" ekranı ve `X-NS-Personel` başlığıyla');
    s.push('çalışan bir imza sistemi vardı (`ns_personel_imzasi()`). Gerçek');
    s.push('giriş kurulunca bu **güvenilmez** hale gelir — giriş yapmış biri');
    s.push('o başlığı değiştirip başkasının adına iş yapabilir. Bunu şimdi');
    s.push('düzelt: `ns_personel_imzasi()` artık `X-NS-Personel` başlığına değil');
    s.push('`auth.uid()`\'ye baksın (`auth_id` üzerinden `kullanicilar`\'a);');
    s.push('"personel seç" ekranını sök, üst çubuktaki kişi çipi artık giriş');
    s.push('yapan kişiyi göstersin, "Personel değiştir" yerine "Çıkış" gelsin.');
    s.push('`personel.js`\'i tamamen silme — çağıran onlarca yer var, onun');
    s.push('yerine oturumdan okuyan ince bir kabuğa çevir.');
    s.push('');

    s.push('- **Kullanıcı listesi.** Admin kullanıcı ekler, siler, pasife alır.');
    s.push('  Pasife alınca o kişinin Auth hesabı da kapatılsın (banned/');
    s.push('  disabled) — yoksa işten ayrılan biri girmeye devam eder.');
    s.push('- **Katman ataması.** Yeni kullanıcı eklenirken şu katmanlardan');
    s.push('  biri seçilir: ' + roller.map(r => '**' + r + '**').join(', ') + '.');
    s.push('- **Şube ataması (varsa).** Bu şablonda bir şube/lokasyon tablosu');
    s.push('  varsa (uygulamanın kendi Ayarlar ekranından kuruluyor, kodda');
    s.push('  sabit değil), kullanıcı eklenirken katmanın yanında bir şube');
    s.push('  de seçilsin — listeyi sabit yazma, o tablodan oku. Kullanıcı');
    s.push('  girişte kendi şubesine bağlanmış sayılsın. Şubeye göre veri');
    s.push('  kısıtlaması (yalnız kendi şubesini görsün) şimdi değil,');
    s.push('  ilerideki "Yetkilendirme" aşamasında kurulacak.');
    ...
    s.push('  (ve varsa şube) gönderilince bu fonksiyon `auth.admin.createUser()`');
    s.push('  ile hesabı açsın, `kullanicilar`\'a `auth_id` üzerinden **upsert**');
    s.push('  yazsın (yukarıdaki trigger\'ın açtığı geçici satırla çakışmasın).');
  ...
  yetkiKur(projeId) {
    ...
    s.push('- **Arayüzde gizlemek kolaylıktır, güvenlik değildir.** Asıl iş');
    s.push('  sunucu tarafındadır. Düğmeyi gizlemen kimseyi engellemez —');
    s.push('  isteği doğrudan atabilir.');
    s.push('- **Var olan kapıları GEVŞETME.** Kurulu sistemde politikalar zaten');
    s.push('  kapalı doğuyor ve `ns_gecerli_kullanici()`, `ns_ust_katman_mi()`');
    s.push('  gibi kapılardan geçiyor. Bir politikayı yeniden yazarken bu');
    s.push('  kapıları düşürme, yerine `true` yazma. Yalnız **üstüne daralt**.');
    s.push('  Bir politikayı gevşetmen gerektiğini düşünüyorsan önce bana sor.');
    s.push('- **RLS\'i sen çalıştıramıyorsun ama SQL sohbette kalmayacak.**');
    s.push('  Önce depodaki göç klasörüne **yeni numaralı bir göç dosyası**');
    s.push('  yaz ve commit\'e gönder (var olan bir göç dosyasını asla');
    s.push('  değiştirme — değişmez kural: yazılmış SQL düzeltilmez, yeni göç');
    s.push('  açılır). Sonra **aynı dosyanın içeriğini** kopyalanabilir tek bir');
    s.push('  blok olarak cevabına da koy — ben Supabase SQL Editör\'e');
    s.push('  yapıştırıp çalıştıracağım. Böylece hem kurulur hem depoda kaydı');
    s.push('  kalır; bir ay sonra o projede ne kurulu olduğu belli olur.');
    s.push('- Depoda `guvenlik.json` varsa `veritabani_surumu` alanını yeni göç');
    s.push('  numarasına göre güncelle — aynı commit\'te.');
    s.push('- Katman/şube kontrolü yapan fonksiyonları (`ns_katman`,');
    s.push('  `ns_izin` vb.) **`stable`** yaz, `immutable` değil — `immutable`');
    s.push('  kalırsa Postgres sonucu sorgu planına gömer, yetki kontrolü');
    s.push('  sessizce hep aynı cevabı verir. `security definer` de gerekiyorsa');
    s.push('  ekle.');
    ...
    s.push('kontrolü) bu kuralların gerçekten tuttuğunu ziyaretçi, personel ve');
    s.push('sunucu işlevi saldırılarıyla ölçecek. Orada bir açık çıkarsa');
    s.push('düzeltme yine **yeni bir göç** olarak yazılır. Onun için aşağıdaki');
