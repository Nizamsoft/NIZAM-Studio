...
function yapiSayfasi(p, d) {
  ...
  /* Yapıyı kurmaya basınca doğrudan ağaç açılır: ara ekran yok. */
  if (AUTH.yonetici) return yapiAkisi(p, d);
    ...
    + (AUTH.yonetici ? `
      <div class="yapi-arac">
        ...
        <button class="sayfa-dug ikincil" data-eylem="yapi-akis-ac" data-proje="${p.id}" type="button">
          ${svg(ICON.katman, 15)} Yeni modül kur</button>
      </div>` : '');
}
...
function yapiTaslak(p) {
  if (!YAPI_TASLAK[p.id]) {
    YAPI_TASLAK[p.id] = { yer: 0, modul: '', anlat: '', kararlar: [],
                          ...
                          mod: 'agac', odak: null, duzelt: null,
                          sayfalar: [], kunye: {} };
  }
  const t = YAPI_TASLAK[p.id];
  /* Kurulu tek modül varsa ağaç onunla açılır: her seferinde seçtirmenin
     anlamı yok. */
  if (!t.modul && !t.acildi) {
    t.acildi = true;
    const kurulu = DB.modulleri(p.id).filter(m => m.ad !== GENEL_MODUL);
    if (kurulu.length === 1) modulYukle(p, t, kurulu[0].ad);
  }
  return t;
}
...
/* Künye tek tek soruluyor: her ekranda bir soru, altında canlı önizleme.
   Sıra kodlama sırası: önce ekranın türü, sonra tuttuğu veri, sonra
   yapılabilenler, en sonda kim ne yapar. */
const KUNYE_ADIM = [
  { anahtar: 'amac',     ad: 'Amaç',     soru: 'Bu sayfa ne işe yarıyor?' },
  { anahtar: 'tur',      ad: 'Tür',      soru: 'Nasıl bir ekran?' },
  { anahtar: 'kalip',    ad: 'Kalıp',    soru: 'Bunlardan biri senin durumun mu?' },
  { anahtar: 'alanlar',  ad: 'Alanlar',  soru: 'Bu sayfa neyi tutuyor?' },
  { anahtar: 'eylemler', ad: 'Eylemler', soru: 'Burada ne yapılabilir?' },
  { anahtar: 'roller',   ad: 'Roller',   soru: 'Kimler görebilir?' },
  { anahtar: 'yetki',    ad: 'Yetki',    soru: 'Kim ne yapabilir?' },
  { anahtar: 'kural',    ad: 'Kural',    soru: 'Uyulması gereken bir kural var mı?' },
  { anahtar: 'ozet',     ad: 'Onay',     soru: 'Doğru anlamış mıyım?' },
];

/* Bir alt adım cevaplanmış sayılır mı? İleri düğmesi buna bakıyor. */
function kunyeAdimTam(k, anahtar, roller) {
  if (anahtar === 'amac')     return !!(k.amac || '').trim();
  if (anahtar === 'tur')      return !!k.tur;
  if (anahtar === 'kalip')    return kalipTam(k);
  if (anahtar === 'alanlar')  return (k.alanlar || []).length > 0
    && k.alanlar.every(a => a.tur !== 'Seçenek' || (a.degerler || []).filter(Boolean).length);
  if (anahtar === 'eylemler') return (k.eylemler || []).length > 0;
  if (anahtar === 'roller')   return (k.roller || []).length > 0;
  if (anahtar === 'yetki')    return (k.eylemler || []).every(x => (k.yetki[x] || []).length);
  return true;
}

/* Yetki adımı ancak anlamlıysa sorulur: tek rol varsa sormanın anlamı yok. */
function kunyeAdimlari(k, roller) {
  return KUNYE_ADIM.filter(a =>
    a.anahtar !== 'yetki' || ((k.eylemler || []).length && roller.length > 1));
}

function yapiAdimlari(t, p) {
  /* Artık yalnız künye alt adımları: modül ve sayfalar ağaçtan kuruluyor. */
  const roller = rolListesi(((p && p.palet) || {}).roller);
  if (!t.duzelt || !t.sayfalar.includes(t.duzelt)) return [];
  const k = yapiKunye(t, t.duzelt);
  return kunyeAdimlari(k, roller).map(ka => ({
    tur: 'kunye', sayfa: t.duzelt, alt: ka.anahtar, ad: t.duzelt,
    obek: t.duzelt, obekNot: 'Bu ekranı kurmak için gerekenler.',
    aciklama: ka.soru }));
}

/* Yapı durağı doğrudan ağaçla açılır: modül ve sayfalar buradan kurulur,
   bir sayfaya dokununca o sayfa kökün yerine geçip kendi dallarını açar. */
function yapiAkisi(p, d) {
  const t = yapiTaslak(p);
  if (t.mod === 'anlat') return anlatEkrani(p, t);
  if (t.mod === 'kunye' && t.duzelt && t.sayfalar.includes(t.duzelt))
    return kunyeEkrani(p, t);
  t.mod = 'agac';
...
/* ---- Ağaç ---- */
const DAL_RENK = {
  amac: '#c9ced6', tur: '#8d8378', kalip: '#3d9970', alanlar: '#c9ced6',
  eylemler: '#c9a227', roller: '#5b8def', yetki: '#5b8def', kural: '#d0a13c',
};

function dalOzeti(k, anahtar, roller) {
  ...
  if (anahtar === 'alanlar') return (k.alanlar || []).length
    ...
    : 'alan yok';
  if (anahtar === 'eylemler') return (k.eylemler || []).length
    ? k.eylemler.map(x => x.toLocaleLowerCase('tr')).join(' · ') : 'eylem yok';
  if (anahtar === 'roller') return (k.roller || []).length
    ? k.roller[0] + ' ve üstü' : 'seçilmedi';
  if (anahtar === 'yetki') {
    const n = Object.keys(k.yetki || {}).length;
    return n ? n + ' eylemin yetkisi belli' : 'belirlenmedi';
  }
  return k.kural || 'henüz yazılmadı';
}

function agacEkrani(p, t) {
  const sayfa = t.odak && t.sayfalar.includes(t.odak) ? t.odak : null;
  return sayfa ? agacSayfa(p, t, sayfa) : agacModul(p, t);
}

function agacKabuk(p, yol, tuval, dugmeler) {
  return `<div class="ag-ekran" style="${renkDegiskenleri(p.renk)}">
    <div class="ag-yol">${yol}</div>
    <div class="ag-govde">${tuval}</div>
    <div class="ag-alt">${dugmeler}</div>
  </div>`;
}

function agacModul(p, t) {
  const kurulu  = DB.modulleri(p.id).filter(m => m.ad !== GENEL_MODUL);
  const harf    = (p.firma || '?').trim().slice(0, 2).toLocaleUpperCase('tr');
  const Y = 22, DY = 72, XS = 172;
  const say = t.sayfalar.length;
  const mY  = Y + Math.max(0, (say - 1)) / 2 * DY;
  const boy = Y + (say + 1) * DY + 40;

  const sayfaNode = t.sayfalar.map((sf, i) => {
    const k = t.kunye[sf] || {};
    const durum = kunyeTam(k) ? 'tamam' : 'eksik';
    return `
      <button class="nd ${durum}" style="left:${XS}px;right:0;top:${Y + i * DY}px;height:54px;
              animation-delay:${(0.05 * i + 0.1).toFixed(2)}s" type="button"
              data-eylem="agac-sayfa" data-proje="${p.id}" data-ad="${esc(sf)}">
        <span class="nokta"></span>
        <b>${esc(sf)}</b><i>${esc(agacSayfaAlt(k))}</i></button>`;
  }).join('');

  const cizgi = t.sayfalar.map((_, i) => `
    <path d="M172 ${Y + i * DY + 27} H ${XS}" stroke="#4a4440" fill="none"/>`).join('');

  const tuval = `<div class="tuval" style="height:${boy}px">
    <svg>
      <defs><linearGradient id="agRenk" x1="0" y1="0" x2="1" y2="0">
        <stop offset="0" stop-color="var(--p1)"/><stop offset="1" stop-color="var(--p2)"/>
      </linearGradient></defs>
      ${t.modul ? `<path d="M42 ${mY + 28} H 54" stroke="url(#agRenk)" stroke-width="1.6" fill="none"/>` : ''}
      ${say ? `<path d="M158 ${mY + 28} H 164 M164 ${Y + 27} V ${Y + (say - 1) * DY + 27}"
                     stroke="#4a4440" fill="none"/>` : ''}
      ${cizgi}
      ${t.modul ? `<path d="M164 ${Y + (say ? (say - 1) * DY : 0) + 27} V ${Y + say * DY + 22}
                          H ${XS}" stroke="#3c3734" stroke-dasharray="4 4" fill="none"/>` : ''}
    </svg>

    <button class="nd kok" style="left:0;top:${mY + 8}px;width:42px;height:42px" type="button"
            data-eylem="agac-kok" data-proje="${p.id}"><b>${esc(harf)}</b></button>

    ${t.modul ? `
      <button class="nd modul secili" style="left:54px;top:${mY}px;width:104px;height:56px"
              type="button" data-eylem="agac-modul" data-proje="${p.id}">
        <b>${esc(t.modul)}</b><i>${say} sayfa</i></button>
      ${sayfaNode}
      <button class="nd hayalet" style="left:${XS}px;right:0;top:${Y + say * DY}px;height:44px;
              animation-delay:${(0.05 * say + 0.14).toFixed(2)}s" type="button"
              data-eylem="yapi-sayfa-yaz" data-proje="${p.id}">
        <b>${svg(ICON.arti, 13)} sayfa ekle</b></button>`
    : `
      <button class="nd hayalet" style="left:54px;top:${mY}px;width:160px;height:56px"
              type="button" data-eylem="agac-modul" data-proje="${p.id}">
        <b>${svg(ICON.arti, 13)} modül kur</b></button>`}
  </div>`;

  const tam = t.modul && say && t.sayfalar.every(sf => kunyeTam(t.kunye[sf]));
  const dugmeler = `
    <button class="ag-dug" type="button" data-eylem="agac-anlat" data-proje="${p.id}">
      ${svg(ICON.kopya, 15)} ${t.sayfalar.length && (t.anlat || '').trim()
        ? 'Anlatımı düzenle' : 'Claude ile kur'}</button>
    <button class="ag-dug ana" type="button" ${tam ? '' : 'disabled'}
            data-eylem="yapi-kur" data-proje="${p.id}">
      ${svg(ICON.check, 15)} Kur</button>`;

  return agacKabuk(p, `<b>${esc(p.firma)}</b><s>›</s>yapı ağacı
    ${kurulu.length ? `<span class="ag-rozet">${kurulu.length} modül kurulu</span>` : ''}`,
    tuval, dugmeler);
}
...
function agacSayfa(p, t, sayfa) {
  const k = yapiKunye(t, sayfa);
  const roller = rolListesi((p.palet || {}).roller);
  const dallar = kunyeAdimlari(k, roller).filter(a => a.anahtar !== 'ozet');
  const Y = 18, DY = 66, XS = 142;
  const mY = Y + Math.max(0, dallar.length - 1) / 2 * DY;
  const boy = Y + dallar.length * DY + 30;

  const dolu = {
    amac: !!(k.amac || '').trim(), tur: !!k.tur, kalip: (k.kalip || []).length,
    alanlar: (k.alanlar || []).length, eylemler: (k.eylemler || []).length,
    roller: (k.roller || []).length, yetki: Object.keys(k.yetki || {}).length,
    kural: !!(k.kural || '').trim(),
  };
  const node = dallar.map((a, i) => {
    /* Boş ama isteğe bağlı olan yeşil görünmesin: "kalıp yok" tamamlanmış
       değil, dokunulmamış demek. */
    const durum = !kunyeAdimTam(k, a.anahtar, roller) ? 'eksik'
      : dolu[a.anahtar] ? 'tamam' : 'bos';
    return `
      <button class="nd dal ${durum}"
              style="left:${XS}px;right:0;top:${Y + i * DY}px;height:52px;
                     animation-delay:${(0.05 * i + 0.08).toFixed(2)}s" type="button"
              data-eylem="agac-dal" data-proje="${p.id}" data-sayfa="${esc(sayfa)}"
              data-ad="${a.anahtar}">
        <span class="rk" style="background:${DAL_RENK[a.anahtar] || '#8d8378'}"></span>
        <span class="nokta"></span>
        <b>${esc(a.ad)}</b><i>${esc(dalOzeti(k, a.anahtar, roller))}</i></button>`;
  }).join('');

  const cizgi = dallar.map((_, i) => `
    <path d="M142 ${Y + i * DY + 26} H ${XS}" stroke="#4a4440" fill="none"/>`).join('');

  const tuval = `<div class="tuval" style="height:${boy}px">
    <svg>
      <path d="M126 ${mY + 26} H 134 M134 ${Y + 26} V ${Y + (dallar.length - 1) * DY + 26}"
            stroke="#4a4440" fill="none"/>
      ${cizgi}
    </svg>
    <button class="nd modul secili" style="left:0;top:${mY - 4}px;width:126px;height:64px"
            type="button" data-eylem="agac-geri" data-proje="${p.id}">
      <b>${esc(sayfa)}</b><i>${esc(agacSayfaAlt(k))}</i></button>
    ${node}
  </div>`;
  ...
  const dugmeler = `
    <button class="ag-dug" type="button" data-eylem="agac-sayfa-sil"
      ...
      ${svg(ICON.kapat, 14)} Sayfayı kaldır</button>
    <button class="ag-dug guclu" type="button" data-eylem="agac-geri" data-proje="${p.id}">
      ${svg(ICON.chevron, 14)} Ağaca dön</button>`;

  return agacKabuk(p, `<b>${esc(t.modul)}</b><s>›</s>${esc(sayfa)}
    <button class="ag-geri" type="button" data-eylem="agac-geri" data-proje="${p.id}"
            title="Ağaca dön">${svg(ICON.chevron, 14)}</button>`, tuval, dugmeler);
}

/* Anlat ve künye ekranları: eski akış iskeletini kullanır. */
function anlatEkrani(p, t) {
  const adim = { tur: 'anlat', ad: 'Anlat', obek: esc(t.modul || 'Modül'),
    obekNot: 'Claude sana soracak, sonucu buraya yapıştıracaksın.',
  ...
  ONIZLEME_KUNYE = null; ONIZLEME_EKRAN = 'panel';
  ONIZLEME_ADIM = 'gezinme'; ONIZLEME_CIHAZ = 'web';
  return `<div class="akis yapi y-anlat">
    <div class="adim-serit">
      <div class="as-obek"><span class="as-rozet">${svg(ICON.kopya, 13)}</span>
        <span class="as-ad"><b>${adim.obek}</b><i>${esc(adim.obekNot)}</i></span>
        <button class="ab-kararlar" type="button" data-eylem="agac-geri" data-proje="${p.id}"
                title="Ağaca dön">${svg(ICON.kapat, 14)}</button></div>
    </div>
    <div class="onz-satir"><div class="onz-goz">${onizlemeIc(p, p.palet)}</div></div>
    <div class="akis-alt">
      <div class="adim-bas"><div class="ab-yazi"><b>${esc(adim.ad)}</b>
        <i>${esc(adim.aciklama)}</i></div></div>
      ${yapiGovde(p, t, adim)}
      <div class="adim-gez">
        <button class="ag geri" type="button" data-eylem="agac-geri" data-proje="${p.id}">
          ${svg(ICON.chevron, 14)} Ağaca dön</button>
        <button class="ag ileri" type="button" data-eylem="agac-geri" data-proje="${p.id}">
          Bitti ${svg(ICON.chevron, 14)}</button>
      </div>
    </div>
  </div>`;
}

function kunyeEkrani(p, t) {
  const adm = yapiAdimlari(t, p);
  ...
  if (adim.tur === 'kunye') {
    ...
    ONIZLEME_ADIM  = 'kunye';
    if (k.tur || (k.alanlar || []).length) {
      ONIZLEME_KUNYE = Object.assign({ sayfa: adim.sayfa }, k);
    }
  } else {
  ...
  return `<div class="akis yapi y-${adim.tur}${adim.alt ? ' k-' + adim.alt : ''}">
    <div class="adim-serit">
      <div class="as-obek">
        <span class="as-rozet">${no + 1}</span>
        <span class="as-ad"><b>${esc(adim.obek)}</b><i>${esc(adim.obekNot)}</i></span>
        <button class="ab-kararlar" type="button" data-eylem="agac-geri" data-proje="${p.id}"
                title="Ağaca dön">${svg(ICON.kapat, 14)}</button>
      </div>
      <div class="as-noktalar"><span class="as-grup suan" style="flex:1">${adm.map((_, i) => `
        <button class="${i === no ? 'on' : i < no ? 'gecti' : ''}" type="button"
                data-eylem="yapi-adim" data-proje="${p.id}" data-deger="${i}"><i></i></button>`)
        .join('')}</span></div>
    ...
    <div class="akis-alt">
      ${kunyeYolu(t, adim, p)}
      ${yapiGovde(p, t, adim)}
      ${yapiGezinme(p, no, adm, t, adim)}
    </div>
  </div>`;
}

/* Sayfa adı + hangi sorudayız. Nokta şeridi bu sayfaya ait. */
function kunyeYolu(t, adim, p) {
  const k = yapiKunye(t, adim.sayfa);
  const roller = rolListesi((p.palet || {}).roller);
  const liste = kunyeAdimlari(k, roller);
  const i = liste.findIndex(x => x.anahtar === adim.alt);
  return `
    <div class="ky-ust">
      <span class="ky-yol"><b>${esc(adim.sayfa)}</b><s>›</s>${esc(liste[i].ad)}</span>
      <span class="ky-adimlar">${liste.map((x, j) =>
        `<i class="${j === i ? 'on' : j < i ? 'gecti' : ''}"></i>`).join('')}</span>
    </div>`;
}
...
function yapiGovde(p, t, adim) {
  if (adim.tur === 'anlat') {
    ...
    return `<div class="kunye-kaydir">
      ${balon('Bu modülde ne olacağını bana anlat. Konuşur gibi yaz — '
        + 'ekranlar, hangi bilgiler tutulacak, neyin neyi etkilediği.',
        'Sonra bunu Claude\'a çözümletip yapıyı kuruyorum; kalan soruları sana sorarım.')}
      <label class="field ky-alan">
        <textarea id="anlat-kutu" rows="7" spellcheck="false"
          placeholder="Örn. Muhasebe modülünde hesaplar sayfası olacak. 100-Kasa, 102-Banka gibi ana hesaplar, altlarında 102.01 gibi alt hesaplar. Alt hesaba basınca hareketleri açılsın. Bir kayıt üç hesabı birden etkileyebilir…"
          data-anlat="${p.id}">${esc(t.anlat || '')}</textarea>
      </label>
      <div class="palet-dug">
        <button class="sayfa-dug ${dolu ? '' : 'ikincil'}" type="button"
                data-eylem="anlat-prompt" data-proje="${p.id}" ${dolu ? '' : 'disabled'}>
          ${svg(ICON.kopya, 15)} Prompt kopyala</button>
        <button class="sayfa-dug ikincil" type="button"
                data-eylem="anlat-aktar" data-proje="${p.id}">
          ${svg(ICON.ice, 15)} Cevabı yapıştır</button>
      </div>
      ${t.sayfalar.length ? `<p class="ky-not">${svg(ICON.tik, 12)}
        ${t.sayfalar.length} sayfa aktarıldı${(t.kararlar || []).length
          ? ' · ' + t.kararlar.length + ' karar kaydedildi' : ''}.</p>` : `
        <p class="ky-not">Claude önce sana soru soracak; anlaştıktan sonra verdiği
        bloğu buraya yapıştır. Anlatmak istemiyorsan boş geç.</p>`}
    </div>`;
  }

  return kunyeGovde(p, t, adim);
}

/* ---- Künyenin alt adımları ---- */
function kunyeGovde(p, t, adim) {
  ...
  if (adim.alt === 'amac') {
    ...
    return `<div class="kunye-kaydir">
      ${balon('<em>' + esc(sf) + '</em> ne işe yarıyor? Kendi cümlenle söyle.',
              'Bu cümle AI\'ın ekranı neden yaptığını anlamasını sağlıyor.')}
      <label class="field ky-alan">
  ...
  if (adim.alt === 'tur') {
    return `<div class="kunye-kaydir">
      ${balon('Bu sayfa nasıl bir ekran olacak?', 'Seçtiğin türün iskeleti üstte canlanır.')}
  ...
  if (adim.alt === 'kalip') {
    const secili = k.kalip || [];
    return `<div class="kunye-kaydir">
      ${balon('Bu sayfa bunlardan birine benziyor mu?',
              'Benziyorsa seç — yapıyı ben kurarım, tek tek anlatmana gerek kalmaz.')}
      <div class="raf">${KALIP.map(kl => `
        <button class="bsc ${secili.includes(kl.anahtar) ? 'on' : ''}" type="button"
  ...
  if (adim.alt === 'roller') {
    if (!roller.length) return `<div class="kunye-kaydir">
      ${balon('Önce rolleri girmen gerekiyor.',
              'Firma durağında rol katmanlarını yazınca burası dolar.')}</div>`;
    const taban = k.roller.length ? roller.findIndex(r => k.roller.includes(r)) : -1;
    return `<div class="kunye-kaydir">
      ${balon('Bu sayfayı en alttan kim görebilmeli?',
              'Alttakini seçince üstündekiler kendiliğinden gelir — amir personelin işini görür.')}
      <div class="rol-merdiven">${roller.map((r, i) => {
        const secili = taban > -1 && i >= taban;
        const oto = secili && i > taban;
        return `<button class="rm ${secili ? 'on' : ''} ${oto ? 'oto' : ''}" type="button"
                        data-eylem="yapi-ky-rol" ${veri} data-ad="${esc(r)}">
          <span class="rm-kat mono">${i + 1}</span>
          <span class="rm-ad"><b>${esc(r)}</b><i>${
            oto ? esc(roller[i - 1]) + '’i gördüğü için otomatik'
            : secili ? 'seçtiğin taban katman' : 'göremez'}</i></span>
          <span class="rm-tik">${secili ? svg(ICON.tik, 11) : ''}</span></button>`;
      }).join('')}</div>
    </div>`;
  }
  ...
  if (adim.alt === 'yetki') {
    return `<div class="kunye-kaydir">
      ${balon('Görmek ayrı, yapmak ayrı. Hangi eylemi kim yapabilir?',
              'Alttakini seçince üstü otomatik gelir.')}
      <div class="yetki">${k.eylemler.map(ey => {
        const secik = k.yetki[ey] || [];
        const taban = secik.length ? k.roller.findIndex(r => secik.includes(r)) : -1;
        return `<div class="yt"><b>${esc(ey)}</b><div class="cipler">
          ${k.roller.map((r, i) => `
            <button class="${taban > -1 && i >= taban ? 'on' : ''}" type="button"
                    data-eylem="yapi-ky-yetki" ${veri} data-ey="${esc(ey)}"
  ...
  if (adim.alt === 'kural') {
    const ornek = ['Onaylanan kayıt değiştirilemez.', 'Tutar eksi olamaz.',
                   'Silme yerine pasife alınır.', 'Aynı ad iki kez girilemez.'];
    return `<div class="kunye-kaydir">
      ${balon('Bu sayfada uyulması gereken bir kural var mı?',
              'En çok yanlış anlaşılan yer burası. Yoksa boş bırak.')}
      <label class="field ky-alan">
        <input type="text" data-ky="kural" ${veri} value="${esc(k.kural)}"
               maxlength="200" autocomplete="off"
               placeholder="Örn. Onaylanan sipariş değiştirilemez.">
      </label>
      <div class="ky-bas">Sık kullanılanlar</div>
      <div class="ornekler">${ornek.map(x => `
        <button type="button" data-eylem="yapi-ky-kural-ornek" ${veri}
                data-ad="${esc(x)}">${esc(x)}</button>`).join('')}</div>
    </div>`;
  }

  /* Onay — anladığımı tekrar ediyorum */
  const alanSatiri = k.alanlar.map(a => a.ad.toLocaleLowerCase('tr')
    + (a.tur === 'Seçenek' && (a.degerler || []).length
        ? ' (' + a.degerler.join(', ') + ')' : '')).join(', ');
  return `<div class="ozet-kaydir">
    ${balon('Anladığımı bir kez tekrar edeyim, yanlışım varsa geri dön düzelt.')}
    <div class="ozet-bal">
      <p>${kunyeCumlesi(sf, k, true)}</p>
      <div class="satir"><u>Tablo</u><span>${esc(tabloAdi(sf))} — ${esc(
        k.alanlar.map(a => sutunAdi(a.ad)).join(', ') || '—')}</span></div>
      <div class="satir"><u>Alanlar</u><span>${esc(alanSatiri || '—')}</span></div>
      <div class="satir"><u>Güvenlik</u><span>${esc(
        k.roller.join(' · ') || '—')}${k.eylemler.length
          ? ' — ' + k.eylemler.map(ey => ey.toLocaleLowerCase('tr') + ': '
              + ((k.yetki[ey] || k.roller)[0] || '—')).join(', ') : ''}</span></div>
    </div>
  </div>`;
}
...
function yapiGezinme(p, no, adm, t, adim) {
  const son = no === adm.length - 1;
  const roller = rolListesi((p.palet || {}).roller);
  /* Cevaplanmadan ilerlenmiyor: yarım künye AI'a tahmin ettiriyor. */
  const kilit = !kunyeAdimTam(yapiKunye(t, adim.sayfa), adim.alt, roller);
  const yazi = adim.alt === 'kural' && !(yapiKunye(t, adim.sayfa).kural || '').trim()
    ? 'Kural yok, geç' : adim.alt === 'kalip'
      && !(yapiKunye(t, adim.sayfa).kalip || []).length ? 'Hiçbiri, geç' : 'İleri';

  return `
    <div class="adim-gez">
      <button class="ag geri" type="button"
              data-eylem="${no ? 'yapi-adim' : 'agac-geri'}" data-proje="${p.id}"
              data-deger="${no - 1}">
        ${svg(ICON.chevron, 14)} Geri</button>
      ${son
        ? `<button class="ag ileri" type="button" ${kilit ? 'disabled' : ''}
                   data-eylem="agac-geri" data-proje="${p.id}">
             ${svg(ICON.check, 14)} Bitti</button>`
        : `<button class="ag ileri" type="button" ${kilit ? 'disabled' : ''}
                   data-eylem="yapi-adim" data-proje="${p.id}" data-deger="${no + 1}">
             ${esc(yazi)} ${svg(ICON.chevron, 14)}</button>`}
    </div>`;
}
...
function yapiBaglari() {
  ...
  $$('[data-ky]').forEach(el => {
    ...
    el.addEventListener('input', () => {
      ...
      if (!pr) return;
      yapiKunye(yapiTaslak(pr), el.dataset.sayfa)[el.dataset.ky] = el.value;
      yapiIleriTazele(pr);
    });
  });
}
...
/* Çipe basınca bütün sayfayı yeniden çizmiyoruz: kaydırma yerinden
   oynuyordu. Yalnız önizleme ve İleri düğmesi tazelenir. */
function yapiOnizlemeTazele(pr) {
  const t = yapiTaslak(pr);
  const adm = yapiAdimlari(t, pr);
  const adim = adm[Math.min(t.yer, adm.length - 1)];
  if (!adim || adim.tur !== 'kunye') return;
  const k  = yapiKunye(t, adim.sayfa);
  const tp = SAYFA_TURU.find(x => x.ad === k.tur);
  ONIZLEME_EKRAN = tp ? tp.ekran : 'liste';
  ONIZLEME_KUNYE = (k.tur || (k.alanlar || []).length)
    ? Object.assign({ sayfa: adim.sayfa }, k) : null;
  const goz = $('.onz-goz');
...
function yapiIleriTazele(pr) {
  ...
  const adim = adm[Math.min(t.yer, adm.length - 1)];
  const dug = $('.ag.ileri');
  if (!dug || !adim || adim.tur !== 'kunye' || dug.classList.contains('kur')) return;
  dug.disabled = !kunyeAdimTam(yapiKunye(t, adim.sayfa), adim.alt,
    rolListesi((pr.palet || {}).roller));
}
...
function cozumlemeUygula(t, cozum, p) {
  ...
  cozum.sayfalar.forEach(sf => {
    ...
    if (!k.roller.length) {
      const gelen = Array.isArray(sf.roller) ? sf.roller.filter(r => roller.includes(r)) : [];
      const taban = gelen.length ? Math.min(...gelen.map(r => roller.indexOf(r))) : 0;
      k.roller = roller.slice(taban);
    }
    k.eylemler.forEach(ey => {
      if (k.yetki[ey]) return;
      const gelen = (sf.yetki && Array.isArray(sf.yetki[ey]))
        ? sf.yetki[ey].filter(r => k.roller.includes(r)) : [];
      const taban = gelen.length ? Math.min(...gelen.map(r => k.roller.indexOf(r))) : 0;
      k.yetki[ey] = k.roller.slice(taban);
    });
  });
}
...
async function eylemCalistir(el) {
  ...
  if (e === 'agac-geri') {
    ...
    if (!pr) return;
    const t = yapiTaslak(pr);
    if (t.mod === 'kunye' || t.mod === 'anlat') { t.mod = 'agac'; t.duzelt = null; }
    else t.odak = null;
    render();
    $('#view').scrollTop = 0;
    return;
  ...
  if (e === 'agac-sayfa') {
    const pr = DB.proje(el.dataset.proje);
    if (!pr) return;
    yapiTaslak(pr).odak = el.dataset.ad;
    render();
    $('#view').scrollTop = 0;
    return;
  }
  ...
  if (e === 'agac-dal') {
    ...
    if (!pr) return;
    const t = yapiTaslak(pr);
    t.duzelt = el.dataset.sayfa;
    t.mod = 'kunye';
    const i = yapiAdimlari(t, pr).findIndex(a => a.alt === el.dataset.ad);
    t.yer = i > -1 ? i : 0;
    render();
    $('#view').scrollTop = 0;
    return;
  ...
  if (e === 'yapi-adim') {
    ...
    const t = yapiTaslak(pr);
    const hedef = Number(el.dataset.deger);
    /* Künye bölümünden çıkınca düzeltme kipi kapanır: liste yine kontrol
       ekranı olur. */
    const adm = yapiAdimlari(t, pr);
    if (t.duzelt && (!adm[hedef] || adm[hedef].tur !== 'kunye')) t.duzelt = null;
    t.yer = hedef;
    render();
    $('#view').scrollTop = 0;
    return;
  ...
  if (e === 'yapi-sayfaya-git') {
    const pr = DB.proje(el.dataset.proje);
    if (!pr) return;
    const t = yapiTaslak(pr);
    t.duzelt = el.dataset.ad;
    const i = yapiAdimlari(t, pr).findIndex(a => a.tur === 'kunye');
    t.yer = i > -1 ? i : t.yer;
    render();
    $('#view').scrollTop = 0;
    return;
  ...
  if (e === 'yapi-ky-eylem') {
    ...
    const k = yapiKunye(yapiTaslak(pr), el.dataset.sayfa);
    const ad = el.dataset.ad;
    const i = k.eylemler.indexOf(ad);
    if (i > -1) { k.eylemler.splice(i, 1); delete k.yetki[ad]; }
    else { k.eylemler.push(ad); k.yetki[ad] = (k.roller || []).slice(); }
    el.classList.toggle('on', k.eylemler.includes(ad));
    yapiOnizlemeTazele(pr); yapiIleriTazele(pr);
    return;
  }

  /* Rol merdiveni: alttakini seçmek üstündekileri de kapsar — amir
     personelin işini görür, ayrıca işaretlemeye gerek yok. */
  if (e === 'yapi-ky-rol') {
    const pr = DB.proje(el.dataset.proje);
    if (!pr) return;
    const roller = rolListesi((pr.palet || {}).roller);
    const k = yapiKunye(yapiTaslak(pr), el.dataset.sayfa);
    const i = roller.indexOf(el.dataset.ad);
    if (i < 0) return;
    const taban = k.roller.length ? roller.findIndex(r => k.roller.includes(r)) : -1;
    k.roller = taban === i ? [] : roller.slice(i);
    /* Eylem yetkileri rol listesinin dışına taşamaz. */
    Object.keys(k.yetki).forEach(ey => {
      k.yetki[ey] = (k.yetki[ey] || []).filter(r => k.roller.includes(r));
      if (!k.yetki[ey].length) k.yetki[ey] = k.roller.slice();
    });
    render();
    return;
  }
  ...
  if (e === 'yapi-ky-yetki') {
    ...
    const i = k.roller.indexOf(el.dataset.ad);
    if (i < 0) return;
    const secik = k.yetki[ey] || [];
    const taban = secik.length ? k.roller.findIndex(r => secik.includes(r)) : -1;
    k.yetki[ey] = taban === i ? [] : k.roller.slice(i);
    const satir = el.parentElement;
    $$('button', satir).forEach((b, j) => b.classList.toggle('on',
      k.yetki[ey].includes(k.roller[j])));
    yapiIleriTazele(pr);
    return;
  }
  ...
  if (e === 'yapi-ky-ornek' || e === 'yapi-ky-kural-ornek') {
    const pr = DB.proje(el.dataset.proje);
    if (!pr) return;
    const alan = e === 'yapi-ky-ornek' ? 'amac' : 'kural';
    yapiKunye(yapiTaslak(pr), el.dataset.sayfa)[alan] = el.dataset.ad;
    const kutu = $(`[data-ky="${alan}"]`);
